...
public class MeasurementManager : MonoBehaviour
{
    ...
    public void WriteSubjectDataToJson()
    {
        // Get the directory for the subject data from the config manager 
        string jsonDirectoryPath = configManager.subjectDataFolderPath;
        
        // Construct an individual filename for the subject data with id and datetime 
        // No ":" in windows filenames, date formatted accordingly 
        string fileName = "\\SubjectID_" + configManager.currentSubjectData.subjectId + "_Datetime_" + configManager.currentSubjectData.dateTimeCreated.Replace(" ","_") + ".json";
        ...
        // Write subject data 
        Debug.Log("[MeasurementManager] Writing subject data at " + filePath + " to disk.");
        File.WriteAllText(filePath,serialized);
        Debug.Log("[MeasurementManager] Writing subject data to disk finished.");
    }    
    */ 
    ...
    private IEnumerator RecordData()  // orig: RecordControllerTriggerAndPositionData
    {
        ...
        while (true)
        {
            ...
            /*
            // Raycast to get hit object
            RaycastHit hitInfoCombined;
            if (Physics.Raycast(dataPoint.eyePositionCombinedWorld, dataPoint.eyeDirectionCombinedWorld, out hitInfoCombined))
            {
                dataPoint.hitObjectNameCombinedEyes = hitInfoCombined.collider.name;
                Debug.Log("***");
                Debug.Log("Combined: " + hitInfoCombined.collider.name);
                ...
                debugLineRenderer.SetPosition(1, hitInfoCombined.point);
                Debug.Log("***");
                dataPoint.hitPointOnObjectCombinedEyes = hitInfoCombined.point;
                dataPoint.hitObjectCenterInWorldCombinedEyes = hitInfoCombined.collider.bounds.center;
            }
            */
            ...
            if (configManager.measureLeftAndRightEyesAdditionallyToCombinedGazeRay)
            {
                ...
                if (Physics.Raycast(dataPoint.eyePositionLeftWorld, dataPoint.eyeDirectionLeftWorld, out hitInfoLeft))
                {
                    dataPoint.hitObjectNameLeftEye = hitInfoLeft.collider.name;
                    //print(hitInfoLeft.collider.name);
                    dataPoint.hitPointOnObjectLeftEye = hitInfoLeft.point;
                    dataPoint.hitObjectCenterInWorldLeftEye = hitInfoLeft.collider.bounds.center;
                }
                */ 
                ...
                if (Physics.Raycast(dataPoint.eyePositionRightWorld, dataPoint.eyeDirectionRightWorld,
                    out hitInfoRight))
                {
                    dataPoint.hitObjectNameRightEye = hitInfoRight.collider.name;
                    //print(hitInfoRight.collider.name);
                    dataPoint.hitPointOnObjectRightEye = hitInfoRight.point;
                    dataPoint.hitObjectCenterInWorldRightEye = hitInfoRight.collider.bounds.center;
                }
                */ 
