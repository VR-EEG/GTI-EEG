...
public class CsvIO : MonoBehaviour
{
    ...
    // Read a file from supplied path 
    // Returns full text string 
    public string ReadFileFromPath(string filePath)
    {
        
        // Check whether file at filePath exists
        if (!File.Exists(filePath))
        {
            Debug.Log("[CsvIO] File at " + filePath.ToString() + " does not exist, not reading!");
            return null;
        }
        ...
        // Return file content 
        Debug.Log("[CsvIO] Read file at " + filePath.ToString() + ".");
        return content;
    }
